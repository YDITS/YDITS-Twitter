...
class YditsTwitter:
    def __init__(self, *, consumer_key: str, consumer_secret: str, access_token: None | str=None, access_token_secret: None | str=None) -> None:
        self.get_date()
        ...
        if ((access_token is None) or (access_token_secret is None)):
            oauth_tokens = self.connection_setup(consumer_key=consumer_key,
                           consumer_secret=consumer_secret)
            access_token = oauth_tokens["oauth_token"]
            access_token_secret = oauth_tokens["oauth_token_secret"]

        self.client = OAuth1Session(consumer_key,
    ...
    def connection_setup(self, *, consumer_key: str, consumer_secret: str) -> dict:
        ...
        get_access_token = twitter_api.AccessToken(consumer_key=consumer_key,
                                consumer_secret=consumer_secret,
                                owner_key=owner_key,
    ...
    def upload(self, content, eew_isFinal):
        if self.eew_tree != "":
            data = {
                'text': content,
                'repry': {
                    'in_reply_to_tweet_id': self.eew_tree
                }
            }
        else:
            data = {
        ...
        if response.status_code == 201:
            ...
            self.error(errCode=0x0221, line=self.frame.f_lineno, errContent=response.status_code)
